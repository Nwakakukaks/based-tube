  ...
  return (
    <div className="h-[88vh]">
      ...
      <div className="flex flex-col w-full my-4 flex-grow h-[70%] overflow-y-auto">
        {messages.map((msg, index) => (
          <div key={index} className={`flex justify-${msg.sender === "user" ? "end" : "start"} w-full mb-3`}>
            <div
              className={`flex items-start space-x-2 ${msg.sender === "user" ? "bg-red-200" : "bg-primary"} p-2 rounded-lg shadow`}
            >
      ...
      <div className="flex items-center w-full mt-4">
        ...
        />
        <Button onClick={handleSend} className="ml-2 bg-red-200 hover:bg-red-100 text-black rounded-full py-2 px-5">
          <p> Send message</p>
        </Button>
      </div>
    </div>
