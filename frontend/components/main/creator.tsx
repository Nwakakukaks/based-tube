...
const Creator: React.FC = () => {
  ...
  const submitHistoricalAddress = async () => {
    if (historicalAddress) {
      ...
      try {
        const response = await fetch('/api/fetch-messages', {
          ...
          body: JSON.stringify({ address: historicalAddress }),
        });
        const data = await response.json();
        setLoading(false);
        if (data.error) {
          ...
          setDataDisplay(data.transactions);
        }
      } catch (error) {
        setLoading(false);
    ...
    <div className="text-center bg-gray-100 p-4">
        ...
        <div className="left-column p-6">
          ...
          <div className="mt-6 flex justify-around">
            ...
            <div className="feature p-4 bg-white rounded-lg shadow-lg">
              ...
              <p>Input your YouTube Live URL and Aptos address, then share the generated link with your viewers on your live.</p>
            </div>
          </div>
        </div>

        <div className="right-column p-6">
      ...
      <div className="analytics-dashboard mt-6 bg-white p-4 rounded shadow">
        ...
        <div className="flex justify-around mt-4">
          <div className="analytics-input-box">
          ...
          <div className="analytics-input-box">
            <h3 className="text-xl">Historical chat data</h3>
            <p>Input your address to see superchats and payments from previous YouTube live streams.</p>
            <div className="flex mt-2">
              <input
                type="text"
                ...
                className="flex-1 p-2 border rounded"
              />
              <button className="analytics-button ml-2 bg-red-600 text-white p-2 rounded" onClick={submitHistoricalAddress}>
                Submit
              </button>
            </div>
          </div>
          ...
          <div className="data-display mt-4">
            ...
            <ul>
              {dataDisplay.map((message, index) => (
                <li key={index} className="mt-2">{message}</li>
              ))}
            </ul>
          </div>
        )}
